function [highcost,s_info]=LVDC(R,code,trellis,L,simpVit,key)
...
%     disp(['  t= ',int2str(t)])
    ...
    for s_i=1:sNo%                                       k--------i------------j           for i (for j (for k)))
        ...
        %88888888888888888888888888888888888 arranging inputs to s_i and restrict them to L ones
        ...
        %888888888888888888888888888 NEW 888888888888888888888888888888888888888        
%         for in=1:min(L, length(s_info(s_i,t).instates))%removing the input paths to s_i that don't obey the key
%             if key(length(s_info(s_i,t).path(in).ST))~=0 && key(length(s_info(s_i,t).path(in).ST))~=s_i
%                 s_info(s_i,t).instates(in)=[];
%                 s_info(s_i,t).intime(in)=[];
%                 s_info(s_i,t).cost(in)=[];
%                 s_info(s_i,t).path(in)=[];
%             end            
%         end
        %8888888888888888888888888888888888888888888888888888888888888888
        if L< length(s_info(s_i,t).instates)% constrain input numbers to L ones
        ...
        for j=1:trellis(s_i).outNo
            ...
            for k=1:(length(s_info(s_i,t).instates)-simpVit*(length(s_info(s_i,t).instates)-1))                                                 
                    ...
                    if key(nexstNo)  %if key is not zero (jump)
                        s_info(s_j,tj).path(ind).ST=[s_info(s_i,t).path(k).ST key(nexstNo)];
                    else
